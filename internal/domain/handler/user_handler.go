...
func (h *UserHandler) RegisterUserList(c *gin.Context) {
	var registerUserList []entity.ReqisterViewModel
	err := c.ShouldBindJSON(&registerUserList)
	if err != nil {
		response.ResponseError(c, err.Error(), http.StatusInternalServerError)
		return
	}

	for _, user := range registerUserList {
		registerUserError := user.Validate()
		if len(registerUserError) > 0 {
			response.ResponseCustomError(c, registerUserError, http.StatusBadRequest)
			return
	...
	result, err := h.userService.SaveUserList(&registerUserList)
	if err != nil {
		response.ResponseError(c, err.Error(), http.StatusInternalServerError)
		return
	}
